...
export function RedotpayRecentUsers() {
  ...
  const generateUsers = () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      list.push({
        id: `user-${Date.now()}-${i}-${Math.random()}`,
  ...
  useEffect(() => {
    setIsMounted(true);
    const savedData = localStorage.getItem('redotpay_recent_data');
    const savedTime = localStorage.getItem('redotpay_recent_time');
    const now = Date.now();
    ...
    if (savedData && savedTime && (now - parseInt(savedTime)) < 60000) {
      ...
      setUsers(newData);
      localStorage.setItem('redotpay_recent_data', JSON.stringify(newData));
      localStorage.setItem('redotpay_recent_time', now.toString());
    }
    ...
    const interval = setInterval(() => {
      ...
      setUsers(newData);
      localStorage.setItem('redotpay_recent_data', JSON.stringify(newData));
      localStorage.setItem('redotpay_recent_time', Date.now().toString());
    }, 60000);
    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-background relative overflow-hidden border-t border-border/50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-10">
          ...
          >
            Real-time tracking of global users joining Redotpay, completing verification, and securing their cards.
          </motion.p>
        </div>

        <div className="bg-card border border-border rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 shadow-sm overflow-hidden transform-gpu">
            ...
            <AnimatePresence mode="popLayout">
              {users.map((user) => (
                <motion.div 
                  ...
                  className="flex items-center justify-between p-3.5 sm:p-4 bg-background border border-border/70 rounded-xl hover:border-primary/40 transition-colors transform-gpu"
                >
                  <div className="flex flex-col gap-1.5">
                    <span className="text-sm sm:text-base font-black text-foreground tracking-wide">{user.uid}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] sm:text-xs text-muted font-bold flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {user.hasCard && (
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary/10 rounded-xl flex items-center justify-center text-primary border border-primary/20 shadow-sm" title="Card Activated">
                        <FaCreditCard className="w-4 h-4 sm:w-5 sm:h-5" />
                      </div>
                    )}
                    {!user.isVerified && !user.hasCard && (
                      <span className="text-[10px] sm:text-xs font-bold text-muted bg-secondary px-3 py-1.5 rounded-lg border border-border shadow-sm">
                        Pending
                      </span>
                    )}
                  </div>

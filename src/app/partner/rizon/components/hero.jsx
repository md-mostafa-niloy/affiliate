...
export function RizonHero() {
  ...
  const inviteLink = "https://www.getrizon.com/referral-link?name=SARFENAZ";
  const appLink = "https://play.google.com/store/apps/details?id=com.rizon.app&hl=en";
  ...
  return (
    <section className="relative w-full flex items-center justify-center pt-8 pb-6 lg:pt-12 lg:pb-10 overflow-hidden bg-background">
      ...
      <div className="max-w-5xl mx-auto px-3 sm:px-6 relative z-10 flex flex-col items-center text-center w-full">
        ...
        >
          Get Massive <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">90% Discount</span> on Rizon
        </motion.h1>
        ...
        >
          Unlock Rizon Emerald Subscription and premium crypto features with our exclusive verified discount codes instantly.
        </motion.p>
        ...
        >
          {[
            { label: 'Invite Code', code: 'TWKFW', type: 'ref', highlight: false },
            { label: '90% Coupon', code: 'NR', type: 'c1', highlight: true }
          ].map((item) => (
            <div 
              key={item.type}

...
export function RizonRecentUsers() {
  ...
  const generateUsers = () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      list.push({
        id: `rizon-user-${Date.now()}-${i}-${Math.random()}`,
  ...
  useEffect(() => {
    setIsMounted(true);
    const savedData = localStorage.getItem('rizon_recent_data');
    const savedTime = localStorage.getItem('rizon_recent_time');
    const now = Date.now();
    ...
    if (savedData && savedTime && (now - parseInt(savedTime)) < 60000) {
      ...
      setUsers(newData);
      localStorage.setItem('rizon_recent_data', JSON.stringify(newData));
      localStorage.setItem('rizon_recent_time', now.toString());
    }
    ...
    const interval = setInterval(() => {
      ...
      setUsers(newData);
      localStorage.setItem('rizon_recent_data', JSON.stringify(newData));
      localStorage.setItem('rizon_recent_time', Date.now().toString());
    }, 60000);
    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-background relative overflow-hidden border-t border-border/50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-10">
          ...
          >
            Recently Registered <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">Rizon Users</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 10 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Real-time synchronization of users accessing Rizon networks, passing KYC profiles, and unlocking premium features.
          </motion.p>
        </div>
              ...
              {users.map((user) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {user.hasCard && (
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary/10 rounded-xl flex items-center justify-center text-primary border border-primary/20 shadow-sm" title="Emerald Subscribed">

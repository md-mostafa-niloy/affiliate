...
export function TevauRecentUsers() {
  ...
  const generateUsers = () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      list.push({
        id: `tevau-user-${Date.now()}-${i}-${Math.random()}`,
  ...
  useEffect(() => {
    setIsMounted(true);
    const savedData = localStorage.getItem('tevau_recent_data');
    const savedTime = localStorage.getItem('tevau_recent_time');
    const now = Date.now();
    ...
    if (savedData && savedTime && (now - parseInt(savedTime)) < 60000) {
      ...
      setUsers(newData);
      localStorage.setItem('tevau_recent_data', JSON.stringify(newData));
      localStorage.setItem('tevau_recent_time', now.toString());
    }
    ...
    const interval = setInterval(() => {
      ...
      setUsers(newData);
      localStorage.setItem('tevau_recent_data', JSON.stringify(newData));
      localStorage.setItem('tevau_recent_time', Date.now().toString());
    }, 60000);
    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-background relative overflow-hidden border-t border-border/50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-10">
          ...
          >
            Real-time tracking of global users joining Tevau, verifying profiles, and ordering discounted cards.
          </motion.p>
        </div>
